...
const CarDetails = () => {
  ...
  const { id } = useParams()
  const { cars, axios, pickupDate, setPickupDate, returnDate, setReturnDate, phone, setPhone, pickupService, setPickupService, pickupLocation, setPickupLocation, dropService, setDropService, dropLocation, setDropLocation, pickupTime, setPickupTime, returnTime, setReturnTime, user, setShowLogin, setAuthModalReason } = useAppContext()
  ...
  const handleSubmit = async (e) => {
    ...
    // 5️⃣ Check if user is logged in
    if (!user) {
      // Save booking data
      setPendingBooking({
        car: id,
  ...
  useEffect(() => {
    // Only reset form if there's no pending booking (user hasn't tried to book yet)
    if (!pendingBooking) {
      setPickupDate("")
      setReturnDate("")
      setPhone("")
  ...
  // Show message after login if there was a pending booking
  useEffect(() => {
    if (user && pendingBooking) {
      toast.success('Welcome back! Please click "Request Booking" to complete your reservation')
      setPendingBooking(null) // Clear the flag after showing message
    }
  }, [user])
